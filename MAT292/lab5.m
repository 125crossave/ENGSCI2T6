...
% The solutions will all decay while oscillating, except for y = 0

%
% (c) Solve the DE and write the exact solution. Explain why this justifies
% your answer in (b).

%y(t) = e^(-t/2)*(c1*cos(2t) + c2*sin(2t))
%This solution makes sense since it will oscilate around x = 0, but decays
%towards 0 over time for any value of c1 and c2.
...
%
% a) graph shown in attached pdf

% b) Solutions grow without oscillating, except c1 = 0 in the general soln below and y = 0

%c) The general soln is y = c1*e^((2 - sqrt(3))*(t/2)) + c2 *e^((-2-sqrt(3))*(t/2)).
%The e term related to c1 will dominate as t grows large, which means that
%the sign of c1 will determine whetehr the solution approaches positive or
%negative infinity. The e term related to c2 is insignificant in comparison
%since it appraoches 0 as t grows large. When c1 = 0, the soln decays
%towards 0 since the e term related to c2 is the only one left.
...
%a) graph shown in attached pdf
%b) Solns decay without oscillating, except y = 0
%c) The general soln is y = c1*e^((sqrt(2) - sqrt(3))*(t/2)) + c2 *e^((-sqrt(2)-sqrt(3))*(t/2)).
% Due to the negative exponents of both terms, the solution approaches 0 as t grows large, and this will happen regardless
% of the values of c1 and c2 (provided both are not 0). This agrees with the fact that all solutions
% (save y = 0) are decaying without oscillations
%% Example
%
% Consider the ODE:
%
...
p = [1 2 6 2 5];
roots(p)
% The general solution is (e^(-t))(Acos2t
% + Bsin2t) + Ccost + Dsint. where ABCD are constants.
...
%We expect all solutions to oscillate without either growing or decaying,
%because as t grows large the Ccost + Dsint term will dominate, which
% just oscillates around y =0. However, if C and D are zero and A and/or
%B are non-zero, then the solution would decay to 0 due to the exponential
%e^(-t) term.
%% Exercise 5
%
...
%
% There are no major differences between my solution and that provided by
%IODE; the differences between the two solutions are very slight and hardly
%noticeable. graph is shown in the attached PDF
p = @(t)exp(-t/5);
q = @(t)(1-exp(-t/5));
g = @(t)sin(2*t);
[t,y] = DE2_rizzmaster(p,q,g, 0, 20, 1,0, 0.01);
plot(t,y, 'color', 'm');
title("DE2_rizzmaster + euler solutions to Exercise 7 2nd order IVP");
legend('DE2_rizzmaster', 'Location', 'best');